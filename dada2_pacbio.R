...
main <- function(CFG){
  ...
  fq_files <- dadautils::list_filepairs(CFG$input_path, verify=F) #%>%
    #dadautils::verify_filepairs()
  print(fq_files)
  sample.names <- dadautils::extract_sample_names(fq_files, rule="basename")
  
  remove_primers(fq_files, primer.fwd=CFG$dada2_removePrimers$primer.fwd, 
                 primer.rev=dada2:::rc(CFG$dada2_removePrimers$primer.rev), orient=TRUE)
  ...
  fq_files_no_primer <- dadautils::list_filepairs(file.path(CFG$input_path, "no_primers"), verify=F)
  ...
  filtN_files <- dadautils::list_filepairs(filtN_path, verify=F) 
...
if (!interactive()){
  ...
  cfgfile = "/mnt/storage/data/edna/dada/projects/robin_foo/pacbio/dada2pacbio.yaml"
}
