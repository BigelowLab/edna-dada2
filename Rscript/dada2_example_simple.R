...
main <- function(
  cfg = "/home/btupper/edna/edna-dada2/config/dada2_example.yml"){
  
  RETURN <- 0
  CFG <- dadautils::get_configuration(cfg)
  ######
  CFG$multithread <- dadautils::count_cores()
  ...
  LOGFILE <- file.path(CFG$output_path, "log")
  ok <- flog.appender(appender.tee(LOGFILE))
  ...
  flog.info("filter and trim of input files with multithread = %i", CFG$multithread)
  ...
  filtN_files <- sapply(fq_files,
    function(flist){
      file.path(filtN_path, dadautils::add_extension(basename(flist), ext = ".gz", no_dup = TRUE) )
    }, simplify = FALSE)
  
  filtN_r <- filterAndTrim(
                          fq_files$forward, 
                          filtN_files$forward,
                          rev           = fq_files$reverse, 
                          filt.rev      = filtN_files$reverse,
  ...
  flog.info("learn errors with multithread = %i", CFG$multithread)
  ...
  flog.info("learn errors *reverse* with multithread = %i", CFG$multithread)
  cat(paste("  ", filtN_files$reverse), sep = "\n", file = LOGFILE, append = TRUE)
  reverse <-  learnErrors(filtN_files$reverse, multithread = CFG$multithread)
  
  flog.info("learn errors *forward* with multithread = %i", CFG$multithread)
  cat(paste("  ", filtN_files$forward), sep = "\n", file = LOGFILE, append = TRUE)
  forward <-  learnErrors(filtN_files$forward, multithread = CFG$multithread)
  ...
  flog.info("run dada with multithread = %i", CFG$multithread)
  
  dada_r <- list(
    forward = dada2::dada(filtN_filest$forward, err$forward, multithread = multithread),
    reverse = dada2::dada(filtN_files$reverse, err$reverse, multithread = multithread)
  )
